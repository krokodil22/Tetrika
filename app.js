// ===== Игра v30 с уровнями 1–7 и фейерверком на финале =====
const LEVELS = [
...
// === Fireworks for final level ===
let fwCanvas=null, fwCtx=null, fwRAF=null, fwParticles=[], fwLastBurst=0;
function startFireworks(){
  if(fwCanvas) return;
  fwCanvas = document.createElement('canvas');
  fwCanvas.width = elBoard.clientWidth;
  fwCanvas.height = elBoard.clientHeight;
  Object.assign(fwCanvas.style, {position:'absolute', inset:'0', zIndex:'5', pointerEvents:'none'});
  elBoard.appendChild(fwCanvas);
  fwCtx = fwCanvas.getContext('2d');
  fwParticles = []; fwLastBurst = 0;
  const loop = (t)=>{
    fwRAF = requestAnimationFrame(loop);
    if(fwCanvas.width !== elBoard.clientWidth || fwCanvas.height !== elBoard.clientHeight){
      fwCanvas.width = elBoard.clientWidth; fwCanvas.height = elBoard.clientHeight;
    }
    fwCtx.clearRect(0,0,fwCanvas.width, fwCanvas.height);
    if(!fwLastBurst || t - fwLastBurst > 600){ fwLastBurst = t; spawnBurst(); }
    updateParticles();
  };
  fwRAF = requestAnimationFrame(loop);
}
function stopFireworks(){
  ...
  if(fwCanvas && fwCanvas.parentNode) fwCanvas.parentNode.removeChild(fwCanvas);
  fwCanvas = null; fwCtx = null; fwParticles = [];
}
function spawnBurst(){
  const cx = Math.random()*elBoard.clientWidth*0.9 + elBoard.clientWidth*0.05;
  const cy = Math.random()*elBoard.clientHeight*0.5 + elBoard.clientHeight*0.1;
  const colors = ['#ff5252','#ffb020','#7cde2b','#2bd0ff','#b770ff'];
  const color = colors[(Math.random()*colors.length)|0];
  const count = 40;
  for(let i=0;i<count;i++){
    const angle = (i/count)*Math.PI*2 + Math.random()*0.2;
    const speed = 1.2 + Math.random()*2.2;
    fwParticles.push({ x:cx, y:cy, vx:Math.cos(angle)*speed, vy:Math.sin(angle)*speed,
      life: 60 + (Math.random()*20)|0, color, size: 2 + Math.random()*2 });
  }
}
...
document.querySelectorAll('.toolbox .block').forEach(block=>{
  block.addEventListener('dragstart', e=>{
    ...
    e.dataTransfer.setData('text/cmd', cmd);
    e.dataTransfer.setData('text/plain', 'CMD:'+cmd);
    e.dataTransfer.effectAllowed='copy';
  });
  block.addEventListener('click', ()=>{
    const insertAt = (placeholderIndex===null)?program.length:placeholderIndex;
    program.splice(insertAt,0,block.dataset.cmd);
...
function renderProgram(){
  ...
  for(let i=0;i<total;i++){
    ...
    li.addEventListener('dragstart', e=>{ e.dataTransfer.setData('text/progIndex', String(idx)); e.dataTransfer.setData('text/plain', 'PROG:'+idx); e.dataTransfer.effectAllowed='move'; });
  ...
  // spacer
  const sp=document.createElement('li'); sp.className='spacer'; sp.setAttribute('aria-hidden','true'); elProgram.appendChild(sp);
}
...
let _dragRAF=null, _pendingIdx=null;
elProgram.addEventListener('dragover', e=>{
  ...
  _dragRAF = requestAnimationFrame(()=>{
    _dragRAF=null;
    if(placeholderIndex!==_pendingIdx){ placeholderIndex=_pendingIdx; renderProgram(); }
  });
});
elProgram.addEventListener('drop', e=>{
  e.preventDefault();
  let cmd=e.dataTransfer.getData('text/cmd');
  let fromStr=e.dataTransfer.getData('text/progIndex');
  if(!cmd && !fromStr){
    const plain=e.dataTransfer.getData('text/plain')||'';
    if(plain.startsWith('CMD:')) cmd=plain.slice(4);
    else if(plain.startsWith('PROG:')) fromStr=plain.slice(5);
  }
  const insertAt=(placeholderIndex===null)?program.length:placeholderIndex;
  if(cmd){
...
async function loadLevel(index=0, opts={preserveProgram:false}){ stopFireworks();
  ...
  const renderH=Math.round(availableH); // v31: same visual height for all levels
  const renderW=Math.round(natW * (renderH / natH));
...
async function execCommand(cmd){
  ...
  if(cmd==='LEFT'||cmd==='RIGHT'){
    ...
    if(cmd==='RIGHT'){ state.dir = DIR_ORDER[(i+1)%4]; state.rotDeg=(state.rotDeg||0)+90; }
    else{ state.dir = DIR_ORDER[(i+3)%4]; state.rotDeg=(state.rotDeg||0)-90; }
...
function onSuccess(){
  ...
  if(isLast){
    elSuccessText.textContent = 'Отлично! Ты справился со всеми заданиями!';
...
function hideSuccess(){ elSuccess.classList.add('hidden'); elNext.style.display=''; stopFireworks(); }
