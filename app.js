...
function autoScrollProgram(mouseY){
  const rect = elProgram.getBoundingClientRect();
  const threshold = 36; const speed = 28;
  if(mouseY < rect.top + threshold) elProgram.scrollTop -= speed;
  if(mouseY > rect.bottom - threshold) elProgram.scrollTop += speed;
}
...
elProgram.addEventListener('dragenter', e=>{ e.preventDefault(); });
let _dragRAF=null, _pendingIdx=null, _lastY=0;
elProgram.addEventListener('dragover', e=>{
  e.preventDefault();
  _lastY = e.clientY;
  autoScrollProgram(_lastY);
  const idx=getInsertIndex(elProgram, _lastY);
